...
import { useGetAnalyticsOverviewQuery } from "../Redux/services";
...
function KpiCard({ icon, color, label, value, delta }) {
  return (
    <Box sx={card}>
      ...
      <Typography sx={{ fontSize: "12px", color: GRAY, fontWeight: 600, mb: 0.5 }}>
        {label}
      </Typography>
      <Typography sx={{ fontSize: "24px", fontWeight: 800, color: INK, lineHeight: 1.1 }}>
        {value}
      </Typography>
      {delta && (
        <Typography sx={{ fontSize: "11px", color: delta.up ? GREEN : RUST, fontWeight: 600, mt: 0.5 }}>
          {delta.text}
        </Typography>
      )}
    </Box>
...
const Analytics = () => {
  ...
  const { data, isLoading, isError } = useGetAnalyticsOverviewQuery({ from, to });

  const k = data?.kpis || {};
  const series = data?.timeSeries || [];
  const split = data?.paymentsSplit || { fullPaid: 0, partial: 0 };
  const topTrips = data?.topTrips || [];
  const funnel = data?.funnel || { enquiries: 0, bookings: 0, paid: 0, conversion: 0 };
  const recent = data?.recent || [];

  const pieData = [
    { name: "Full Paid", value: split.fullPaid, color: GREEN },
    { name: "Partial", value: split.partial, color: AMBER },
  ];
  const maxTripRev = Math.max(1, ...topTrips.map((t) => t.revenue));
  const maxFunnel = Math.max(1, funnel.enquiries, funnel.bookings, funnel.paid);
      ...
      {isLoading ? (
        ...
        <Box sx={{ ...card, textAlign: "center", py: 6 }}>
          <Typography sx={{ color: GRAY }}>
            Could not load analytics. Make sure you&apos;re logged in as an admin.
          </Typography>
        </Box>
      ) : (
        <>
          ...
          <Grid container spacing={2} sx={{ mb: 3 }}>
            <Grid item xs={6} sm={4} md={2}>
              <KpiCard icon={<PaymentsOutlinedIcon />} color={GREEN} label="Total Sales" value={fmt(k.totalSales)} />
            </Grid>
            <Grid item xs={6} sm={4} md={2}>
              <KpiCard icon={<HourglassBottomOutlinedIcon />} color={AMBER} label="Total Pending" value={fmt(k.totalPending)} />
            </Grid>
            <Grid item xs={6} sm={4} md={2}>
              <KpiCard icon={<ConfirmationNumberOutlinedIcon />} color={RUST} label="Bookings" value={k.bookings || 0} />
            </Grid>
            <Grid item xs={6} sm={4} md={2}>
              <KpiCard icon={<GroupAddOutlinedIcon />} color={BLUE} label="New Users" value={k.newUsers || 0} />
            </Grid>
            <Grid item xs={6} sm={4} md={2}>
              <KpiCard icon={<ForumOutlinedIcon />} color={PURPLE} label="Enquiries" value={k.enquiries || 0} />
            </Grid>
            <Grid item xs={6} sm={4} md={2}>
              <KpiCard icon={<TrendingUpOutlinedIcon />} color={INK} label="Avg Value" value={fmt(k.avgBookingValue)} />
            </Grid>
          </Grid>
          ...
          <Grid container spacing={2} sx={{ mb: 3 }}>
            <Grid item xs={12} md={8}>
              <Box sx={{ ...card, height: "100%" }}>
                <Typography sx={{ fontSize: "16px", fontWeight: 700, color: INK, mb: 0.5 }}>
                  Revenue Over Time
                </Typography>
                <Typography sx={{ fontSize: "12px", color: GRAY, mb: 2 }}>
                  Collected payments per day
                </Typography>
                {series.length === 0 ? (
                  <Box sx={{ height: 260, display: "grid", placeItems: "center", color: GRAY }}>
                ...
                <Box sx={{ position: "relative" }}>
                  ...
                  <Box sx={{ position: "absolute", inset: 0, display: "grid", placeItems: "center", pointerEvents: "none" }}>
                    <Box sx={{ textAlign: "center" }}>
                      <Typography sx={{ fontSize: "20px", fontWeight: 800, color: INK }}>
                        {fmtShort(k.totalSales)}
                      </Typography>
          ...
          <Grid container spacing={2} sx={{ mb: 3 }}>
            <Grid item xs={12} md={6}>
              <Box sx={{ ...card, height: "100%" }}>
                <Typography sx={{ fontSize: "16px", fontWeight: 700, color: INK, mb: 2 }}>
                  ...
                  topTrips.map((t, i) => (
                    <Box key={i} sx={{ mb: 2 }}>
                      <Box sx={{ display: "flex", justifyContent: "space-between", mb: 0.5 }}>
                        <Typography sx={{ fontSize: "13px", fontWeight: 600, color: INK, maxWidth: "60%", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                          {t.name}
                        </Typography>
                        <Typography sx={{ fontSize: "13px", fontWeight: 700, color: INK }}>
                          {fmt(t.revenue)}
                        </Typography>
                      </Box>
                      ...
                      <Typography sx={{ fontSize: "11px", color: GRAY, mt: 0.3 }}>
            ...
            <Grid item xs={12} md={6}>
              <Box sx={{ ...card, height: "100%" }}>
                ...
                {[
                  { label: "Total Enquiries", value: funnel.enquiries, color: "#E5E7EB", text: INK },
                  { label: "Bookings", value: funnel.bookings, color: "#FBD7CC", text: INK },
                  { label: "Completed (Paid)", value: funnel.paid, color: RUST, text: "#fff" },
                ].map((step, i) => (
                  <Box key={i} sx={{ mb: 1.5 }}>
                    <Box sx={{ display: "flex", justifyContent: "space-between", mb: 0.5 }}>
                ...
                <Box sx={{ textAlign: "center", mt: 2 }}>
                  ...
                  <Typography sx={{ fontSize: "26px", fontWeight: 800, color: RUST }}>{funnel.conversion}%</Typography>
                </Box>
              </Box>
            </Grid>
          </Grid>
              ...
              <Box sx={{ overflowX: "auto" }}>
                <Box sx={{ display: "grid", gridTemplateColumns: "1.5fr 2fr 1fr 1fr 1fr", gap: 1, py: 1, borderBottom: "1px solid #F0F0F0", minWidth: 600 }}>
                  {["User", "Trip", "Amount", "Status", "Date"].map((h) => (
                    <Typography key={h} sx={{ fontSize: "11px", fontWeight: 700, color: GRAY, textTransform: "uppercase", letterSpacing: "0.05em" }}>
                      {h}
                    </Typography>
                  ))}
                </Box>
